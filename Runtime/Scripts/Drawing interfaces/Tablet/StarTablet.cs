...
namespace VRPen {
	...
	public class StarTablet : MonoBehaviour {
		...
		public Transform penTracker;
		public Transform tabletTracker;

		public Vector3 tipOffset = new Vector3(0, 0, -.15f);
		public Matrix4x4 tabletTransform;
		...
		void Update() {
			...
			for (int i = 0; i < n; i++) {
				...
				/*if(pressure > 0)
                {
                    ...
                    penTrackerTransforms.Add((penTracker.transform.localToWorldMatrix));
                    tabletTrackerTransforms.Add((tabletTracker.transform.localToWorldMatrix));
                    Vector3 tabletPoint = new Vector3((point.x - .5f)  * .254f, (point.y - .5f) * 0.15875f, 0);
                    tabletPoints.Add(tabletPoint);

                    if (penTrackerTransforms.Count > 5000)
                    {
                        calibrate();
                        //calibrating = false;
                        //need to clean pentracktransforms
                    }
				...
				//Debug.Log(point);
				//Debug.Log(i+"     "+penValues[i*8+0]+"    "+penValues[i*8+1]+"    "+penValues[i*8+2]+"    "+penValues[i*8+3]+"    "+penValues[i*8+4]+"    "+penValues[i*8+5]+"    "+penValues[i*8+6]+"    "+penValues[i*8+7]+"    ");
			}
			...
			/*for(int x = 0; x < buttonInputs * 9; x++){
                if (getButtonDown(x)){
                    Debug.Log(getButton(x));
                }
            }*/
			...
			/*int[] buttonValues = new int[80];
            int n = readButtons (buttonValues,10);
            buttons = new int[n * 8];
            for (int i = 0; i < 8; i++) {
                pressedEvents[i] = releasedEvents[i] = 0;
            }
            ...
            int[] penValues = new int[40];
            n = readPen (penValues, 10);
            penSamples.Clear ();
            for (int i = 0; i < n; i++) {
                Vector2 point = new Vector2 (penValues [i * 4 + 0] / 39999.0f, 1 -penValues [i * 4 + 1] / 24999.0f);
                float pressure = penValues [i * 4 + 2] / 1023.0f;
                if(pressure > 0 && calibrating)
                {
                ...
                PenState state = (PenState)penValues [i * 4 + 3];
                penSamples.Add (new PenSample (point, pressure, state));
            } */
		}
